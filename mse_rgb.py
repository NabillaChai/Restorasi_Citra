...
# ===== Filter manual 5x5 untuk RGB =====
def min_filter_manual_rgb(img_rgb):
    out = np.zeros_like(img_rgb)
    padded = np.zeros((height+4, width+4, 3), dtype=np.uint8)
    padded[2:-2,2:-2,:] = img_rgb
    for i in range(height):
        for j in range(width):
            for c in range(3):
                kernel = padded[i:i+5,j:j+5,c]
                out[i,j,c] = np.min(kernel)
    return out

def max_filter_manual_rgb(img_rgb):
    out = np.zeros_like(img_rgb)
    padded = np.zeros((height+4, width+4, 3), dtype=np.uint8)
    padded[2:-2,2:-2,:] = img_rgb
    for i in range(height):
        for j in range(width):
            for c in range(3):
                kernel = padded[i:i+5,j:j+5,c]
                out[i,j,c] = np.max(kernel)
    return out

def mean_filter_manual_rgb(img_rgb):
    out = np.zeros_like(img_rgb)
    padded = np.zeros((height+4, width+4, 3), dtype=np.uint8)
    padded[2:-2,2:-2,:] = img_rgb
    for i in range(height):
        for j in range(width):
            for c in range(3):
                kernel = padded[i:i+5,j:j+5,c]
                out[i,j,c] = int(np.sum(kernel)/25)
    return out

def median_filter_manual_rgb(img_rgb):
    out = np.zeros_like(img_rgb)
    padded = np.zeros((height+4, width+4, 3), dtype=np.uint8)
    padded[2:-2,2:-2,:] = img_rgb
    for i in range(height):
        for j in range(width):
            for c in range(3):
                kernel = padded[i:i+5,j:j+5,c].flatten()
                kernel.sort()
                out[i,j,c] = kernel[len(kernel)//2]
    return out

# MSE manual untuk RGB
def mse_manual_rgb(original, filtered):
    total = 0
    for i in range(height):
        for j in range(width):
            for c in range(3):
                total += (int(original[i,j,c]) - int(filtered[i,j,c]))**2
    return total / (height*width*3)
...
for name, level, value, noise_type in noise_list:
    ...
    print("   - Filter Min...")
    mse_min = mse_manual_rgb(img_rgb, min_filter_manual_rgb(noisy_rgb))
    print("   - Filter Max...")
    mse_max = mse_manual_rgb(img_rgb, max_filter_manual_rgb(noisy_rgb))
    print("   - Filter Mean...")
    mse_mean = mse_manual_rgb(img_rgb, mean_filter_manual_rgb(noisy_rgb))
    print("   - Filter Median...")
    mse_median = mse_manual_rgb(img_rgb, median_filter_manual_rgb(noisy_rgb))
...
ax.set_title('Perbandingan Performa Filter terhadap Berbagai Jenis Noise (RGB)', 
