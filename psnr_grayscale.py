...
for name, level, value, noise_type in noise_list:
    ...
    psnr_min = psnr_manual(mse_min)
    
    print("   - Filter Max...")
    mse_max = mse_manual(gray, max_filter_manual(gray_noisy))
    psnr_max = psnr_manual(mse_max)
    
    print("   - Filter Mean...")
    mse_mean = mse_manual(gray, mean_filter_manual(gray_noisy))
    psnr_mean = psnr_manual(mse_mean)
    
    print("   - Filter Median...")
    mse_median = mse_manual(gray, median_filter_manual(gray_noisy))
    psnr_median = psnr_manual(mse_median)

    table_data.append([name, level, psnr_min, psnr_max, psnr_mean, psnr_median])
...
ax.set_title('Perbandingan Performa Filter terhadap Berbagai Jenis Noise (Grayscale)\nBerdasarkan PSNR', 
